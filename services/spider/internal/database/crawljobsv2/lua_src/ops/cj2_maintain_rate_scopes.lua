local registered,code=CJ.Maintenance.register("CJ2_MAINTAIN_RATE_SCOPES")
if not registered then return CJ.Context.reject(code) end
local ctx; ctx,code=CJ.Maintenance.open("CJ2_MAINTAIN_RATE_SCOPES",KEYS,ARGV)
if not ctx then return CJ.Context.reject(code) end
local execution; execution,code=CJ.Maintenance.rate(ctx)
