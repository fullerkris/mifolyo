local registered,code=CJ.Maintenance.register("CJ2_PURGE_RUN_BATCH")
if not registered then return CJ.Context.reject(code) end
local ctx; ctx,code=CJ.Maintenance.open("CJ2_PURGE_RUN_BATCH",KEYS,ARGV)
if not ctx then return CJ.Context.reject(code) end
local execution; execution,code=CJ.Maintenance.purge(ctx)
